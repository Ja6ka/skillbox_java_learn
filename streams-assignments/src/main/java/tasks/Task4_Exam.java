...
public class Task4_Exam {
    ...
    public static TruckType getTypeByWeight(int weight) {
        ...
        } else if (weight <= TruckType.SemiTrailer.maxLoad) {
    ...
    public static Map<TruckType, List<Truck>> groupTrucksByType(List<Truck> trucks) {
        //return trucks.stream().collect(Collectors.groupingBy
        // Тут логику исполнения понял, однако не до конца понимаю как реализовать groupingBy по весу
    ...
    public static Map<TruckType, Long> countTrucksByType(List<Truck> trucks) {
        //return trucks.stream().collect(Collectors.groupingBy(TruckType.values(), Collectors.counting()));
        // Тоже не знаю что задать в качестве первого параметра groupingBy
        throw new PleaseDeleteMeAndImplement();
    }
